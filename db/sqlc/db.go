...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.deleteAccoundStmt, err = db.PrepareContext(ctx, deleteAccound); err != nil {
		return nil, fmt.Errorf("error preparing query DeleteAccound: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.deleteAccoundStmt != nil {
		if cerr := q.deleteAccoundStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing deleteAccoundStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		deleteAccoundStmt:         q.deleteAccoundStmt,
